...
class Screenshot:
    def __init__(self, top, scale):
        ...
        # 保存截图;
        self.img_screenshot.save('./img/test00.png')
        # 打开截图;
        self.img_screenshot = Image.open('./img/test00.png')
        "---------------------------------------------------窗口边框"
        # 初始化边框画布边框宽度;
        ...
        # 间隔固定时间执行指定方法;
        self.enlarge.after(func=self.img_enlarge(), ms=100)
        "---------------------------------------------------键盘监听"
        # 监听所有键盘,并执行参数中方法;
    ...
    def mouse_lift_event(self, event):
        ...
        if down_right_x > 0 and down_right_y > 0:
            self.screenshot_img.save('img/test00.png')
